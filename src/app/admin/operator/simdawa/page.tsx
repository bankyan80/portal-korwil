...
export default function OperatorSimdawa() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/firestore/students?field=schoolId&value=${user.schoolId}`)
      .then(r => r.json())
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <main className="p-6 max-w-7xl mx-auto">
        <h1 className="text-lg font-bold flex items-center gap-2 mb-4"><Users className="w-5 h-5" /> SIMDAWA</h1>
        {loading ? <Loader2 className="w-6 h-6 animate-spin text-blue-600" /> : (
          <p className="text-muted-foreground">Total siswa: {data.length}</p>
        )}
      </main>
    </div>
    </SimpleAdminLayout>
