...
export default function OperatorYatimPiatuPage() {
  ...
  return (
    <AuthGuard requiredRoles={['operator_sekolah', 'super_admin']} requireActive featureName="Data Yatim Piatu">
    <div className="min-h-screen flex flex-col bg-slate-50">
      <header className="sticky top-0 z-50 bg-gradient-to-b from-[#1a5276] to-[#0d3b66]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-3">
            <a href="/admin/operator" className="flex items-center gap-2 text-white/80 hover:text-white transition-colors">
              <ArrowLeft className="w-4 h-4" />
              <span className="text-sm font-medium">Kembali</span>
            </a>
            <div className="flex items-center gap-2">
              <Heart className="w-5 h-5 text-yellow-400" />
              <h1 className="text-sm font-bold text-white uppercase tracking-wide">Data Yatim Piatu</h1>
            </div>
            <div />
          </div>
        </div>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Kelola data siswa yatim piatu • {user?.schoolName || 'Kecamatan Lemahabang'}</p>
        </div>
          ...
          <>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-5">
              ...
              <ul className="text-sm text-gray-600 space-y-1 list-disc list-inside">
                ...
                <li>Data diambil dari database siswa. Jika tidak ditemukan, hubungi admin via WhatsApp.</li>
              </ul>
            </div>
          </>
        )}
