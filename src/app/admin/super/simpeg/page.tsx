  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <header className="bg-gradient-to-b from-[#1a5276] to-[#0d3b66] px-6 py-4 flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-white flex items-center gap-2"><BookOpen className="w-5 h-5" /> SIMPEG</h1><p className="text-sm text-blue-200">{user.displayName}</p></div>
      ...
      <main className="p-6 max-w-7xl mx-auto">
        {loading ? (
          ...
          <div className="bg-white rounded-xl border p-6">
            <p className="text-lg font-semibold">Total Pegawai: {data.length}</p>
          </div>
        )}
      </main>
    </div>
